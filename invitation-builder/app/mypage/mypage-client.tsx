...
export default function MyPageClient({
  ...
  const [activeTab, setActiveTab] = useState<"invitation" | "guestMedia" | "payment">("invitation");
      ...
      <main className="min-h-[calc(100vh-64px)] bg-white px-6 py-12">
        <div className="mx-auto w-full max-w-4xl">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-[#6b7280]">{displayName}님의 제작내역과 결제내역을 관리합니다.</p>
          </div>
          ...
          <div className="mb-4 flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => setActiveTab("guestMedia")}
              className={`inline-flex h-10 items-center rounded-lg px-4 text-sm font-semibold transition-colors ${
                activeTab === "guestMedia"
                  ? "bg-[#111] text-white"
                  : "border border-[#e5e7eb] bg-white text-[#4b5563] hover:bg-[#f8f8f8]"
              }`}
            >
            ...
            <section>
              {guestMedia.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                  {guestMedia.map((item) => (
                    <article key={`${item.url}-${item.uploadedAt}`} className="overflow-hidden rounded-lg border border-[#ececec] bg-white">
                      {item.kind === "image" ? (
                        <Image
                          src={item.url}
                          alt={item.fileName}
